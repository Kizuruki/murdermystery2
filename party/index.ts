...
interface ChatMessage {
  ...
  from: string;
  fromName: string;
  to: "global" | "evil" | string; // string = player id for DM
  text: string;
  timestamp: number;
}
...
interface Player {
  id: string;
  name: string; // character name e.g. "Lord Lochmara"
  nickname: string; // real name e.g. "Alex"
  role: Role;
  originalRole: Role;
  alignment: Alignment;
  alive: boolean;
  ...
  // Clues
  startingClues: Clue[]; // only real/fake known to detective
  discoveredClues: string[]; // texts only, no real/fake label
  sharedClues: string[]; // clue texts this player shared with town
  ...
  // Tracker (stored server-side but only sent to owner)
  trackerMarks: Record<string, "circle" | "x" | "none">;
  trackerRoles: Record<string, string>; // guessed roles
...
interface NightAction {
  ...
  secondaryTargetId?: string | null; // witch next mimic
}
...
interface GameSettings {
  ...
  day1NightSkip: boolean; // for 7-player
}
...
interface GameState {
  ...
  nightActionsExpected: string[]; // player ids who need to submit

  // Voting
  votes: Record<string, VoteAction>;
  judgeSave: string | null; // player id judge is saving
...
const CLUE_TEMPLATES: { key: string; text: (name: string) => string; trueFor: (p: Player) => boolean }[] = [
  {
    key: "civilian",
    text: (n) => `${n} does not play a strong role in the town.`,
    trueFor: (p) => p.role === "Civilian",
  },
  {
    key: "emergency",
    text: (n) => `${n} has knowledge that could be useful in emergencies.`,
    trueFor: (p) => p.role === "Doctor",
  },
  {
    key: "vote",
    text: (n) => `${n} has the power to change the outcome of a vote.`,
    trueFor: (p) => p.role === "Judge",
  },
  {
    key: "secret",
    text: (n) => `${n} might have a reason to act in secret.`,
    trueFor: (p) => ["Murderer", "Assassin", "Spy", "Witch", "Detective"].includes(p.role),
  },
  {
    key: "active_night",
    text: (n) => `${n} is more active at night.`,
    trueFor: (p) => ["Murderer", "Assassin", "Spy", "Witch", "Doctor", "Detective", "Jailor", "Vigilante"].includes(p.role),
  },
  {
    key: "decisive",
    text: (n) => `${n} has a deciding influence in discussions.`,
    trueFor: (p) => p.role === "Mayor",
  },
  {
    key: "shy",
    text: (n) => `${n} is shy in discussions.`,
    trueFor: (p) => !["Mayor", "Judge"].includes(p.role),
  },
  {
    key: "keeps_to_self",
    text: (n) => `${n} keeps to themselves at night.`,
    trueFor: (p) => ["Civilian", "Mayor", "Judge", "Psychic", "Medium", "Coroner"].includes(p.role),
  },
  {
    key: "powerful_day",
    text: (n) => `${n} is powerful during the day.`,
    trueFor: (p) => ["Mayor", "Judge"].includes(p.role),
  },
];
...
function generateStartingClues(player: Player, allPlayers: Player[]): Clue[] {
  ...
  // Fake clue (false for that player)
  found = false;
...
function generatePsychicClue(targetPlayer: Player, allPlayers: Player[], usedTexts: Set<string>): string | null {
  ...
  for (const tmpl of shuffled) {
    ...
    if (tmpl.trueFor(targetPlayer) && !usedTexts.has(text)) {
      return text;
    }
  }
  // fallback: any true clue
  for (const tmpl of shuffled) {
    const text = tmpl.text(targetPlayer.name);
    if (tmpl.trueFor(targetPlayer)) {
      return text;
    }
  }
  return null;
...
function assignRoles(players: Player[], settings: GameSettings): void {
  ...
  // Apply Jailor/Vigilante toggle
  if (!settings.jailorEnabled && settings.vigilanteEnabled) {
    // Replace Jailor with Vigilante if present
...
function assignPreferencesAndObjectives(players: Player[]): void {
  ...
  // Collect all likes/dislikes across players
...
interface NightResult {
  killed: string[]; // player ids
  saved: string[]; // player ids
  jailed: string[]; // player ids
...
function resolveNight(state: GameState): NightResult {
  ...
  const getTarget = (playerId: string) => {
    const a = actions[playerId];
    return a?.targetId ? get(a.targetId) : null;
  };
  ...
  if (assassin && !assassin.assassinUsed && actions[assassin.id]?.targetId && !isJailed(assassin.id)) {
    ...
    // Jailor dies if jailed target is assassinated
  ...
  // ── Murderer ──
  // Skip Night 1 kills in 7-player games
  ...
  if (murderer && !isJailed(murderer.id) && !skip7 && actions[murderer.id]?.targetId) {
    ...
    // Check Witch-Doctor save
    const witchDoctorSaved = witch && witch.witchCurrentMimic === "Doctor" && !isJailed(witch.id) &&
      actions[witch.id]?.targetId === target.id;
    ...
    // Jailor dies if jailed target murdered
    ...
    // Murderer feedback
    const killed = !doctorSaved && !witchDoctorSaved && !result.killed.includes(target.id) === false;
    if (result.killed.includes(target.id) && (result.deathEntries.find(d => d.playerName === target.name && (d.cause === "Murdered")))) {
      result.feedbacks[murderer.id].push(`Your attack on ${target.name} was successful.`);
    } else {
      result.feedbacks[murderer.id].push(`Your attack on ${target.name} was prevented.`);
    }
  ...
  if (spy && !isJailed(spy.id) && !skip7 && actions[spy.id]?.targetId) {
    ...
    if (targetAction?.targetId) {
      const visited = get(targetAction.targetId);
      result.feedbacks[spy.id].push(`${target.name} visited ${visited.name} last night.`);
    } else {
  ...
  if (detective && !isJailed(detective.id) && actions[detective.id]?.targetId) {
    ...
    // Who visited the watched house?
    const visitors = alive.filter(p => {
      if (p.id === detective.id || p.id === watchedId) return false;
      return actions[p.id]?.targetId === watchedId;
    });
  ...
  // ── Witch mimicking night-action roles ──
  if (witch && !isJailed(witch.id) && witch.witchCurrentMimic) {
    ...
    if (mimic === "Spy" && witchTarget) {
      ...
      if (tAction?.targetId) {
        result.feedbacks[witch.id].push(`[As Spy] ${witchTarget.name} visited ${get(tAction.targetId).name} last night.`);
      } else {
        result.feedbacks[witch.id].push(`[As Spy] ${witchTarget.name} did not leave their house last night.`);
      }
    }
    if (mimic === "Detective" && witchTarget) {
      ...
      if (visitors.length > 0) {
        result.feedbacks[witch.id].push(`[As Detective] ${witchTarget.name} was visited by ${visitors.map(v => v.name).join(", ")}.`);
      } else if (leftHouse) {
        result.feedbacks[witch.id].push(`[As Detective] ${witchTarget.name} left their house last night.`);
      } else {
        result.feedbacks[witch.id].push(`[As Detective] No activity at ${witchTarget.name}'s house last night.`);
      }
    }
    if (mimic === "Doctor" && witchTarget) {
      if (result.saved.includes(witchTarget.id)) {
        result.feedbacks[witch.id].push(`[As Doctor] You saved ${witchTarget.name}'s life last night.`);
      } else {
        result.feedbacks[witch.id].push(`[As Doctor] ${witchTarget.name} did not require assistance.`);
      }
    }
    if (mimic === "Jailor" && witchTarget) {
      result.feedbacks[witch.id].push(`[As Jailor] You jailed ${witchTarget.name} last night.`);
    }
    if (mimic === "Medium" && witchTarget) {
      result.feedbacks[witch.id].push(`[As Medium] ${witchTarget.name} was a ${witchTarget.role}.`);
    }
    if (mimic === "Coroner") {
      const recentDead = state.deathLog.filter(d => d.round === state.round - 1);
      if (recentDead.length > 0) {
        result.feedbacks[witch.id].push(`[As Coroner] Deaths from last round: ${recentDead.map(d => `${d.playerName} (${d.cause})`).join(", ")}.`);
      } else {
        result.feedbacks[witch.id].push(`[As Coroner] No deaths last round.`);
      }
    }
  ...
  if (psychic && !isJailed(psychic.id) && actions[psychic.id]?.targetId) {
    ...
    const usedTexts = new Set<string>([
      ...target.startingClues.map(c => c.text),
  ...
  // ── Coroner (sees deaths from PREVIOUS round) ──
  if (coroner && !isJailed(coroner.id)) {
    const prevDeaths = state.deathLog.filter(d => d.round === state.round - 1);
    if (prevDeaths.length > 0) {
      result.feedbacks[coroner.id].push(`Cause of death report: ${prevDeaths.map(d => `${d.playerName} — ${d.cause}`).join("; ")}.`);
    } else {
      result.feedbacks[coroner.id].push(`No deaths to report from the previous night.`);
    }
  }
  ...
  // ── Virtual clue discovery (1/10 chance per player) ──
  ...
  // ── Murderer feedback fix (consolidate) ──
  if (murderer && actions[murderer.id]?.targetId) {
    ...
    result.feedbacks[murderer.id] = [wasKilled
      ? `Your attack on ${target.name} was successful.`
      : `Your attack on ${target.name} was prevented.`
    ];
  }
  if (assassin && actions[assassin.id]?.targetId && assassin.assassinUsed) {
    ...
    result.feedbacks[assassin.id] = [wasKilled
      ? `Your unblockable strike on ${target.name} was successful.`
      : `Your strike on ${target.name} was already dead.`
    ];
  }
...
function checkSuccession(state: GameState): void {
  const aliveMurderers = Object.values(state.players).filter(p =>
    p.alive && (p.originalRole === "Murderer" || p.role === "Murderer")
  );
  if (aliveMurderers.length > 0) return;

  for (const role of SUCCESSION_ORDER) {
    const successor = Object.values(state.players).find(p =>
      p.alive && p.originalRole === role && p.role !== "Murderer"
    );
    if (successor) {
...
export default class MurderMysteryServer implements Party.Server {
  ...
  onMessage(message: string, sender: Party.Connection) {
    ...
    switch (msg.type) {
      ...
      case "host_start_game": {
        ...
        const players = Object.values(state.players);
        if (players.length < 6 || players.length > 16) {
          sender.send(JSON.stringify({ type: "error", message: "Need 6-16 players" }));
          return;
        }
        assignRoles(players, state.settings);
        assignPreferencesAndObjectives(players);
        players.forEach(p => {
          p.startingClues = generateStartingClues(p, players);
        });
      ...
      case "host_pause_timer": {
        ...
        if (!state.timerPaused && state.timerEndsAt) {
          ...
          this.broadcast({ type: "timer_paused" });
        }
        break;
      }
      ...
      case "host_resume_timer": {
        ...
        if (state.timerPaused && state.timerEndsAt && state.timerPausedAt) {
          ...
          this.broadcast({ type: "timer_resumed", endsAt: state.timerEndsAt });
        }
        break;
      }
      ...
      case "night_action": {
        ...
        state.nightActions[sender.id] = {
          playerId: sender.id,
          targetId: msg.targetId,
          secondaryTargetId: msg.secondaryTargetId,
        };
        // Check if all expected actions received
        const submitted = Object.keys(state.nightActions).filter(id =>
          state.nightActionsExpected.includes(id)
        );
        if (submitted.length >= state.nightActionsExpected.length) {
      ...
      case "update_tracker": {
        const p = state.players[sender.id];
        if (p) p.trackerMarks[msg.targetId] = msg.mark;
      ...
      case "update_tracker_role": {
        const p = state.players[sender.id];
        if (p) p.trackerRoles[msg.targetId] = msg.role;
        sender.send(JSON.stringify({ type: "tracker_updated" }));
        break;
      }
      ...
      case "enter_clue_code": {
        ...
        if (!clue) {
          sender.send(JSON.stringify({ type: "error", message: "Invalid or already found code" }));
          return;
        }
        clue.found = true;
      ...
      case "share_clue": {
        ...
        // Add to all other players' feedback logs
        Object.values(state.players).forEach(other => {
          if (other.id !== sender.id) {
            other.feedbackLog.push({
              round: state.round,
              phase: "night",
              text: `📢 ${p.name} shared a clue: "${msg.clueText}"`,
            });
          }
        });
      ...
      case "objective_complete": {
        ...
        // Mark target's obj 3 (someone identified them)
        target.objectivesCompleted[2] = true;
        sender.send(JSON.stringify({ type: "objective_updated", objectivesCompleted: p.objectivesCompleted }));
        this.sendPrivate(msg.targetPlayerId, { type: "objective_updated", objectivesCompleted: target.objectivesCompleted });
        break;
      }
  ...
  startTimer(phase: Phase) {
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (Date.now() >= (this.state.timerEndsAt ?? 0)) {
  ...
  advancePhase() {
    ...
    switch (state.phase) {
      ...
      case "night_feedback":
        if (state.round === 1 && Object.values(state.players).length === 7) {
          // Day 1 for 7-player: skip discussion/voting
          state.phase = "night_action";
          state.round++;
          this.prepareNightActions();
          this.startTimer("night_action");
  ...
  resolveNightAndAdvance() {
    ...
    // Append feedback to player logs
    Object.entries(result.feedbacks).forEach(([pid, msgs]) => {
      const p = state.players[pid];
      if (!p) return;
      msgs.forEach(text => {
        p.feedbackLog.push({ round: state.round, phase: "night", text });
      });
    });
  ...
  resolveVotesAndAdvance() {
    ...
    alive.forEach(voter => {
      ...
      const power = voter.votePower;
      counts[vote.targetId] = (counts[vote.targetId] || 0) + power;
    });
    ...
    if (savedId && candidates.includes(savedId)) {
      ...
      if (judge && state.judgeSave) {
        ...
        if (savedPlayer?.alignment === "Good") {
          judge.feedbackLog.push({ round: state.round, phase: "day", text: `You saved ${savedPlayer.name} from elimination. They were ${savedPlayer.alignment}.` });
        }
      }
      // Give judge vote totals
      const judgeOrWitch = Object.values(state.players).find(p =>
        (p.role === "Judge" || (p.role === "Witch" && p.witchCurrentMimic === "Judge")) && p.alive
      );
      if (judgeOrWitch) {
        const tally = Object.entries(counts).map(([id, n]) => `${state.players[id]?.name}: ${n}`).join(", ");
        judgeOrWitch.feedbackLog.push({ round: state.round, phase: "day", text: `Vote totals: ${tally}. ${state.players[savedId]?.name} was saved.` });
      }
      this.proceedToNight();
      return;
    }
    ...
    if (candidates.length > 1 || maxVotes === 0) {
      ...
      if (judgeOrWitch) {
        const tally = Object.entries(counts).map(([id, n]) => `${state.players[id]?.name}: ${n}`).join(", ");
        judgeOrWitch.feedbackLog.push({ round: state.round, phase: "day", text: `Vote totals: ${tally}. Tie — no elimination.` });
      }
      this.proceedToNight();
      return;
    }
    ...
    if (eliminated) {
      ...
      if (judgeOrWitch) {
        const tally = Object.entries(counts).map(([id, n]) => `${state.players[id]?.name}: ${n}`).join(", ");
        judgeOrWitch.feedbackLog.push({ round: state.round, phase: "day", text: `Vote totals: ${tally}. ${eliminated.name} was eliminated (${eliminated.alignment}).` });
      }

      // Mayor feedback
      const mayor = Object.values(state.players).find(p => p.role === "Mayor" && p.alive);
      if (mayor && state.votes[mayor.id]?.targetId === eliminated.id) {
        mayor.feedbackLog.push({
          round: state.round,
          phase: "day",
          text: `The player you voted for (${eliminated.name}) was eliminated. They were ${eliminated.alignment}.${eliminated.alignment === "Good" ? " Your voting power is reduced to 1 next round." : ""}`,
        });
      ...
      // Witch-Mayor
      const witch = Object.values(state.players).find(p => p.role === "Witch" && p.alive && p.witchCurrentMimic === "Mayor");
      if (witch && state.votes[witch.id]?.targetId === eliminated.id) {
        witch.feedbackLog.push({
          round: state.round,
          phase: "day",
          text: `[As Mayor] The player you voted for was ${eliminated.alignment}.`,
        });
      }
    }
  ...
  proceedToNight() {
    const state = this.state!;
    state.round++;
    // Restore mayor penalty
    Object.values(state.players).forEach(p => {
      if (p.mayorPenaltyNextRound) {
        p.votePower = 2;
        p.mayorPenaltyNextRound = false;
      }
    });
    state.phase = "night_action";
    this.prepareNightActions();
  ...
  createPlayerView(viewerId: string): object {
    ...
    Object.values(state.players).forEach(p => {
      ...
      const canSeeRole = isOwn || isHost ||
        (viewer && EVIL_ROLES.includes(viewer.role) && EVIL_ROLES.includes(p.role));
      ...
      players[p.id] = {
        ...
        ...(isOwn ? {
          likes: p.likes,
          dislikes: p.dislikes,
          startingClues: p.role === "Detective"
            ? p.startingClues // Detective sees real/fake labels
            : p.startingClues.map(c => ({ text: c.text })), // Others just get text
    ...
    return {
      ...
      ...(isHost ? {
        ...
        allNightActions: state.nightActions,
      } : {}),
    };
  }
  ...
  deliverChatMessage(msg: ChatMessage) {
    if (msg.to === "global") {
      ...
      // DM
      this.sendPrivate(msg.to, JSON.stringify({ type: "chat", message: msg }));
      this.sendPrivate(msg.from, JSON.stringify({ type: "chat", message: msg }));
    }
  }
  ...
  createInitialState(hostId: string, code: string): GameState {
    return {
      ...
      settings: {
        ...
        customEvilCount: null,
        customSpecialCount: null,
        day1NightSkip: true,
      },
      nightActions: {},
